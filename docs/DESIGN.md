...
> bulldozed, rather than counting as filled and clearing with its line — without
> that the board sat 60–80% empty forever and runs never ended. The density base
> is **superlinear** `[10, 25, 45]`, because `min()`-scaled synergy had flattened
> the return to a constant 15 per unit and nobody ever upzoned. Parks are owed on
> **population**, not on line count, which was farmable with cheap lines. The
> growth ramp is bounded. Full account, with the numbers, in `BALANCE.md`.
...
Gives −5 to every neighbour. It **blocks its row and its column** — a line
containing blight cannot clear at all — and it is **permanent until bulldozed**.
One cell at an intersection kills two of the twenty lines.

*This reverses v0.2, which had blight count as filled and clear with its line.
Measured, that version gifted free line-completion material to the engine
generating it: the board held at 60–80% empty for a thousand placements, clears
removing cells as fast as placements added them, and the run could never end.
Blight that blocks is the only accumulating pressure in the design. It is the
water problem of v0.2 on purpose — and the difference is that blight is caused by
your own play, arrives gradually, and can be removed.*
...
```
cell = max(0, DENSITY_VALUE[own] + Σ pair_value × min(own_density, neighbour_density))
...
`DENSITY_VALUE` is `[0, 10, 25, 45]` — **superlinear**, and that matters. A linear
base with `min()`-scaled synergy pays exactly 15 per density unit at every
density, and since an upzone fills no new cell it also costs tempo. Flat return
plus a tempo cost means a rational player never upzones: the harness measured 98%
of all harvest happening at density 1, with the core verb dead. Superlinear pays
15 / 17.5 / 20 per unit and revives it, while leaving donor farming dead because
the *neighbour* side is still capped by `min()`.
...
| | |
|---|---|
| **M0** | ✅ pure rules module + tests |
| **M1–M3** | ✅ playable build: drag, rotate, overlap, roads, park, demand, blight, bulldozer, skyline |
| **M4** | ✅ harness and first balance pass — see `BALANCE.md` |
| **M5** | art: real massing-model assets behind `src/render/tiles.ts`; audio, haptics |
